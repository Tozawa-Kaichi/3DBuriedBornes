...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (IsGrounded() || m_jumpCount < m_maxJumpCount)
            {
                m_jumpCount++;
                Jump();
            }
    ...
    //        //この方法でも普通に敵をロックオンして倒せたが、上や下に対して体全体が向くので良くない
    //        //transform.LookAt(m_enemyDetector.Target.transform);

    //        //こっちが授業のやつ
    //        Vector3 dir = m_enemyDetector.Target.transform.position - this.transform.position;
    //        dir.y = 0;//y方向への向きを0にして体を水平に。まずるだけむけさせればよし
